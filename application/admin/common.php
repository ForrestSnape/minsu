...
/**
 * 获取指定年份 指定月份的 月开始结束时间戳
 */
function getTimestampEveryMonth($y, $m){
    if($m == 2){
        $d = $y % 4 == 0 && $y % 100 != 0 || $y % 400 == 0 ? 29 : 28;
    }else{
        $d = in_array($m, [1, 3, 5, 7, 8, 10, 12]) ? 31 : 30;
    }
    return [
 ...
 * 打印调试数据
 */
function printData($data, $objToArray = 1){
    if($objToArray) $data = objToArray($data);
    error_log(print_r($data, true), 3, 'd:\debug\data.txt');
}