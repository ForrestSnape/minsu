...
class Index extends Base
{
     ...
     * ajax获取指定年份每个房间全年利润
     */
    public function ajax_getYearRoomProfit()
    {
        $room_mdl = new RoomModel();
        $order_mdl = new OrderModel();
        ...
        foreach ($rooms as $k => $room) {
            $result[$k]['room'] = $room;
            for ($i = 1; $i <= 12; $i++) {
                $begin_end = getTimestampEveryMonth($year, $i);
                $result[$k]['profit'][] = $order_mdl->getProfitByRoomIdInTime($room['id'], $begin_end['begin'], $begin_end['end']);
            }
        }
        return json($result);
    }
     ...
     * ajax获取指定年份每个平台全年利润
     */
    public function ajax_getYearPlatformProfit()
    {
        $platform_mdl = new PlatformModel();
        $order_mdl = new OrderModel();
        ...
        foreach ($platforms as $k => $platform) {
            $result[$k]['platform'] = $platform;
            for ($i = 1; $i <= 12; $i++) {
                $begin_end = getTimestampEveryMonth($year, $i);
                $result[$k]['profit'][] = $order_mdl->getProfitByPlatformIdInTime($platform['id'], $begin_end['begin'], $begin_end['end']);
            }
        }
        return json($result);
    }
